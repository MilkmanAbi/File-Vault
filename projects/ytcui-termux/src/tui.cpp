...
void TUI::set_mouse_capture(bool on) {
    if (!mouse_enabled_) return;  // terminal never had SGR mouse to begin with
    mousemask(on ? (BUTTON1_PRESSED | BUTTON1_CLICKED |
...
// ─── Top-level render ─────────────────────────────────────────────────────────
void TUI::render(AppState& state, const Library* lib) {
...
// starts at y=2.
// Header row 0 tap zones (must stay in sync with input.cpp's header dispatch
// in stream_handle(), same duplication tradeoff every other tap zone in this
// file already makes): back is columns [0,3), menu is the rightmost 6
// columns. Both are drawn wide and high-contrast on purpose -- these are the
// ONLY way to navigate back or reach Save/Add-to-Playlist/Sleep-Timer on a
// touchscreen, so they don't get to be subtle.
void TUI::stream_header(const AppState& state, const std::string& title,
                         bool show_back, bool show_menu) {
    int W = state.term_w;
    bool uni = TermCaps::get().unicode;
    ...
    attron(COLOR_PAIR(Color::ACCENT) | A_BOLD);
    mvprintw(0, show_back ? 4 : 1, "%s", utf8_truncate("♪ " + title, W - (show_back ? 4 : 1) - (show_menu ? 6 : 0) - 1).c_str());
    attroff(COLOR_PAIR(Color::ACCENT) | A_BOLD);
...
void TUI::render_streamlined(AppState& state, const Library* lib) {
    switch ((StreamScreen)state.stream_screen) {
        ...
        case StreamScreen::MiniMenu:        stream_minimenu(state);             break;
        case StreamScreen::PlaylistPick:    stream_playlist_pick(state);        break;
        case StreamScreen::NewPlaylistName: stream_new_playlist_name(state);    break;
        case StreamScreen::TimerInput:      stream_timer_input(state);          break;
    }
}
...
void TUI::stream_menu(const AppState& state, const Library*) {
    ...
    stream_header(state, "ytcui", /*show_back=*/true);
    ...
    if (H - 2 > top + n)
        center_text(H - 2, W, "tap to open  < quit", COLOR_PAIR(Color::BG) | A_DIM);
    stream_status(state);
}

void TUI::stream_search(const AppState& state) {
    int W = state.term_w, H = state.term_h;
    stream_header(state, "Search", /*show_back=*/false);
...
void TUI::stream_browse(const AppState& state) {
    ...
    stream_header(state, state.stream_section.empty() ? "Browse" : state.stream_section,
                  /*show_back=*/true,
                  /*show_menu=*/!state.stream_on_playlists && !state.results.empty());
    ...
    if (state.stream_on_playlists) {
        ...
        center_text(H - 2, W, "tap to open  < back", COLOR_PAIR(Color::BG) | A_DIM);
        stream_status(state);
        return;
    }
    ...
    if (r.empty()) {
        ...
        center_text(H - 2, W, "< back", COLOR_PAIR(Color::BG) | A_DIM);
        stream_status(state);
        return;
    }
    ...
    center_text(H - 2, W, "tap to play  < back", COLOR_PAIR(Color::BG) | A_DIM);
    stream_status(state);
}
...
void TUI::stream_actions(const AppState& state) {
    ...
    center_text(H - 2, W, "tap to play  < back  menu", COLOR_PAIR(Color::BG) | A_DIM);
    stream_status(state);
}

// Now-playing card: album art, stylized waveform, time, title/artist, transport.
// Transport row is pinned to a FIXED row (H-3), independent of how much
// dynamic content (thumbnail/title/channel/timer) rendered above it -- input.cpp's
// tap handling needs a deterministic y to hit-test against, and content that
// grows/shrinks the header stack would otherwise silently break it every time
// something above changed height. Keep this in sync with stream_handle()'s
// Playing case if this row ever moves.
void TUI::stream_playing(AppState& state) {
    int W = state.term_w, H = state.term_h;
    const Video& v = state.stream_now;
    bool uni = TermCaps::get().unicode;
    stream_header(state, "Now Playing", /*show_back=*/true, /*show_menu=*/true);
    int y = 2;
    // Fixed-position zones: transport row and (optionally) sleep timer sit at
    // the bottom of the screen regardless of how much dynamic content (art,
    // title, waveform) renders above them. The sleep timer appears right above
    // the transport row when active, never squeezed out by content above.
    int transport_y = H - 3;
    if (transport_y < 6) transport_y = 6;  // bare minimum for art+title+transport
    int timer_y = state.sleep_timer_active ? transport_y - 1 : -1;
    int content_bottom = timer_y > 0 ? timer_y - 1 : transport_y - 1;

    // Reset each frame; only set again below if a waveform actually draws.
    // input.cpp checks waveform_y >= 0 before treating a tap as a seek.
    state.waveform_y = -1;

    // Album art (chafa), centered, kept compact so the card stays dense.
    if (state.thumbs_available && !v.id.empty() && content_bottom - y > 8) {
        int art_w = std::min(W - 4, (content_bottom - y - 6) * 2); if (art_w < 6) art_w = W - 4;
        int art_rows = std::max(3, art_w / 2); if (art_rows > content_bottom - y - 5) art_rows = std::max(3, content_bottom - y - 5);
    ...
    // true elapsed/total and fill the bar to match; otherwise we show all-dim
    // with a "connecting..." hint so it's clear the app is working on it, not
    // frozen.
    if (y < content_bottom - 2) {
        ...
        // Tappable once mpv has reported a real duration -- input.cpp maps a
        // tap's x offset within [waveform_x, waveform_x+waveform_w) to a
        // fraction of playback_dur and issues an absolute seek.
        bool has_dur = state.playback_dur > 0.5;
        if (has_dur) { state.waveform_y = y; state.waveform_x = wx; state.waveform_w = wf; }

        bool has_pos = state.playback_pos > 0.01;
        double frac = has_dur ? (state.playback_pos / state.playback_dur) : 0.0;
        if (frac < 0) frac = 0;
        if (frac > 1) frac = 1;
        ...
        for (int i = 0; i < wf; i++) {
            ...
            bool past = has_pos && i < filled;
            if (past) attron(COLOR_PAIR(Color::ACCENT));
            else      attron(COLOR_PAIR(Color::STATS) | A_DIM);
            mvprintw(y, wx + i, "%s", uni ? bars[pidx] : (past ? "=" : "-"));
            if (past) attroff(COLOR_PAIR(Color::ACCENT));
            else      attroff(COLOR_PAIR(Color::STATS) | A_DIM);
        }

        attron(COLOR_PAIR(Color::STATS) | A_DIM);
        std::string elapsed = has_pos ? fmt_time(state.playback_pos) : "0:00";
        mvprintw(y + 1, wx, "%s", elapsed.c_str());
        std::string dur = has_dur ? fmt_time(state.playback_dur)
                        : (v.is_live ? "LIVE"
    ...
    if (y < content_bottom) { center_text(y, W, v.title.empty() ? "Nothing playing" : v.title,
                                 COLOR_PAIR(Color::TITLE) | A_BOLD); y++; }
    if (y < content_bottom && !v.channel.empty()) { center_text(y, W, v.channel, COLOR_PAIR(Color::CHANNEL)); y++; }

    // Sleep timer countdown, pinned at timer_y — never pushed off-screen.
    if (state.sleep_timer_active && timer_y > 0) {
        int mins = state.sleep_timer_remaining_sec / 60;
        int secs = state.sleep_timer_remaining_sec % 60;
        char buf[32];
        snprintf(buf, sizeof(buf), "%s %d:%02d", uni ? "☽" : "zzz", mins, secs);
        center_text(timer_y, W, buf, COLOR_PAIR(Color::STATS) | A_DIM);
    }

    // Transport row: three equal-width tap columns (vol-down / play-pause /
    // vol-up). Seeking lives on the waveform above instead (tap a position,
    // jump there) -- ytcui-dl now resolves adaptive audio by default, which
    // (unlike the old muxed-progressive-only pick) actually supports
    // time-based seeking reliably, so there was no reason to keep it off the
    // transport row's three fixed slots too.
    {
        const char* lbl_left  = "vol -";
        const char* lbl_mid   = state.is_paused ? (uni ? "▶" : ">")
                                                  : (uni ? "❚❚" : "||");
        const char* lbl_right = "vol +";
        int col_w = W / 3;
        int lw = utf8_display_width(lbl_left), mw = utf8_display_width(lbl_mid), rw = utf8_display_width(lbl_right);
        int lx = 0 * col_w + (col_w - lw) / 2; if (lx < 0) lx = 0;
        int mx = 1 * col_w + (col_w - mw) / 2; if (mx < 1 * col_w) mx = 1 * col_w;
        int rx = 2 * col_w + (col_w - rw) / 2; if (rx < 2 * col_w) rx = 2 * col_w;
        attron(COLOR_PAIR(Color::ACCENT) | A_BOLD);
        mvprintw(transport_y, lx, "%s", lbl_left);
        mvprintw(transport_y, mx, "%s", lbl_mid);
        mvprintw(transport_y, rx, "%s", lbl_right);
        attroff(COLOR_PAIR(Color::ACCENT) | A_BOLD);
    }
    center_text(H - 1, W, "< back  menu  tap wave=seek",
                COLOR_PAIR(Color::BG) | A_DIM);
}

// Compact action sheet: Save to Library / Add to Playlist / Sleep Timer,
// opened from Browse (result selected) or Playing via the header Menu≡ tap.
// One row per item, same tap-to-activate pattern as the main Menu screen.
void TUI::stream_minimenu(const AppState& state) {
    int W = state.term_w, H = state.term_h;
    stream_header(state, "Menu", /*show_back=*/true);

    auto items = stream_minimenu_items(state);
    int top = 3;
    for (size_t i = 0; i < items.size(); i++) {
        int y = top + (int)i;
        bool s = (state.stream_minimenu_sel == (int)i);
        if (s) {
            paint_sel_bar(y, 1, W - 2);
            attron(sel_attr() | A_BOLD);
            mvprintw(y, 2, "%s", utf8_truncate(items[i], W - 4).c_str());
            attroff(sel_attr() | A_BOLD);
        } else {
            attron(COLOR_PAIR(Color::TITLE));
            mvprintw(y, 3, "%s", utf8_truncate(items[i], W - 5).c_str());
            attroff(COLOR_PAIR(Color::TITLE));
        }
    }
    center_text(H - 2, W, "tap to choose  < back", COLOR_PAIR(Color::BG) | A_DIM);
    stream_status(state);
}

// Playlist picker for Add-to-Playlist: reuses state.playlist_names, the
// same list App::refresh_playlist_names() builds for normal mode's picker
// ("+ Create new playlist" at index 0, then every playlist in the same
// order as library_.playlists() -- index-to-id mapping in the dispatch
// relies on that order matching exactly). Tapping a name adds the in-view
// video directly -- one tap, no confirm step.
void TUI::stream_playlist_pick(const AppState& state) {
    int W = state.term_w, H = state.term_h;
    stream_header(state, "Add to Playlist", /*show_back=*/true);

    const auto& names = state.playlist_names;
    int top = 3, vis = std::max(1, H - 5);
    int sel = state.stream_minimenu_sel, start = 0;
    if (sel >= vis) start = sel - vis + 1;
    for (int i = 0; i < vis && start + i < (int)names.size(); i++) {
        int idx = start + i, y = top + i; bool s = (idx == sel);
        if (s) { paint_sel_bar(y, 1, W - 2); attron(sel_attr() | A_BOLD);
    ...
    center_text(H - 2, W, "tap to add  < back", COLOR_PAIR(Color::BG) | A_DIM);
    stream_status(state);
}

// Text-entry screen for naming a brand-new playlist, same boxed-input
// styling as stream_search().
void TUI::stream_new_playlist_name(const AppState& state) {
    int W = state.term_w, H = state.term_h;
    stream_header(state, "New Playlist", /*show_back=*/false);
    int by = std::max(3, H / 2 - 2);
    draw_box(by, 1, 3, W - 2, Color::SEARCH_BOX);
    int iw = W - 6;
    std::string q = utf8_truncate(state.new_playlist_name, iw);
    attron(COLOR_PAIR(Color::TITLE));
    mvprintw(by + 1, 3, "%s", q.c_str());
    attroff(COLOR_PAIR(Color::TITLE));
    mvchgat(by + 1, 3 + std::min(utf8_display_width(q), iw - 1), 1, A_REVERSE, 0, nullptr);
    center_text(H - 2, W, "type a name · ⏎ create · esc back", COLOR_PAIR(Color::BG) | A_DIM);
    stream_status(state);
}

// Text-entry screen for the in-app sleep timer. Free-form: "45", "1h30m",
// "90m", "2h" -- parsed by App's dispatch, not here; this just renders
// whatever's typed plus a short format hint.
void TUI::stream_timer_input(const AppState& state) {
    int W = state.term_w, H = state.term_h;
    stream_header(state, "Sleep Timer", /*show_back=*/false);
    int by = std::max(3, H / 2 - 3);
    center_text(by, W, "Stop playback after...", COLOR_PAIR(Color::CHANNEL) | A_DIM);
    draw_box(by + 2, 1, 3, W - 2, Color::SEARCH_BOX);
    int iw = W - 6;
    std::string q = utf8_truncate(state.stream_timer_input, iw);
    attron(COLOR_PAIR(Color::TITLE));
    mvprintw(by + 3, 3, "%s", q.c_str());
    attroff(COLOR_PAIR(Color::TITLE));
    mvchgat(by + 3, 3 + std::min(utf8_display_width(q), iw - 1), 1, A_REVERSE, 0, nullptr);
    center_text(by + 6, W, "e.g. 45m · 1h30m · 2h · 90", COLOR_PAIR(Color::STATS) | A_DIM);
    if (state.sleep_timer_active) {
        int mins = state.sleep_timer_remaining_sec / 60, secs = state.sleep_timer_remaining_sec % 60;
        char buf[48];
        snprintf(buf, sizeof(buf), "currently set: %d:%02d remaining", mins, secs);
        center_text(by + 8, W, buf, COLOR_PAIR(Color::STATS) | A_DIM);
    }
    center_text(H - 2, W, "type · ⏎ set · esc back", COLOR_PAIR(Color::BG) | A_DIM);
    stream_status(state);
}
