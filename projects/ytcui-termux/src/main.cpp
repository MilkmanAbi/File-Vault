...
static void check_for_updates_async() {
    FILE* pipe = popen("curl -fsSL --max-time 2 "
                       "https://raw.githubusercontent.com/MilkmanAbi/ytcui-android/main/VERSION "
...
static void print_help(const Ansi& C) {
    ...
    printf("  %s--volume%s <0-130>    starting volume (default 100)\n", C.GREEN, C.RESET);
    ...
    printf("%sPhone / sleep mode%s\n", C.BOLD, C.RESET);
    printf("  %s--sleep-timer%s <min> stop playback and exit after N minutes (implies --audio-only)\n", C.GREEN, C.RESET);
    printf("  %s--audio-only%s        force audio playback, never open an mpv video window\n", C.GREEN, C.RESET);
    printf("                       %son Termux: holds a wake lock for the session so playback%s\n", C.DIM, C.RESET);
    printf("                       %ssurvives the screen turning off%s\n\n", C.DIM, C.RESET);
    ...
    printf("%sMore%s\n", C.BOLD, C.RESET);
    printf("  project   https://github.com/MilkmanAbi/ytcui-android\n");
...
int main(int argc, char* argv[]) {
    ...
    int  sleep_minutes = 0;           // --sleep-timer <minutes>, 0 = off
    bool audio_only    = false;       // --audio-only
    ytui::PlayerOptions player_opts;  // defaults: vol=100, hwdec=auto, cache=on
    ...
    for (int i = 0; i < (int)args.size(); i++) {
        ...
        if ((a == "--theme" || a == "-t") && i + 1 < (int)args.size()) {
            ...
            if (ui_mode != "streamlined")
                fprintf(stderr, "Note: this Termux build is streamlined-only; --mode %s is ignored.\n", ui_mode.c_str());
#endif
        } else if (a == "--sleep-timer" && i + 1 < (int)args.size()) {
            sleep_minutes = atoi(args[++i].c_str());
            if (sleep_minutes <= 0) {
                fprintf(stderr, "Invalid --sleep-timer: must be a positive number of minutes\n");
                return 1;
            }
            audio_only = true;  // a sleep timer implies audio-only; see --help
    ...
    app.set_sleep_options(sleep_minutes, audio_only);  // no-op unless --sleep-timer/--audio-only passed
    g_app = &app;
