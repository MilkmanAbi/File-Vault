...
struct PlayerOptions {
    ...
    int  volume            = 100;    // default volume (--volume)
};
...
public:
    ...
    bool toggle_pause();
    // mpv's own confirmed pause state (via IPC observe_property) is the
    // source of truth whenever we have it -- paused_ is only the optimistic
    // shadow toggle_pause() sets before that confirmation arrives, same
    // pattern get_volume() already uses for the IPC-reported volume.
    bool is_paused() const {
        if (ipc_.connected()) return ipc_.paused();
        return paused_;
    }
    ...
    bool seek_to(double seconds);   // absolute, e.g. from a waveform tap
...
private:
    ...
    bool playing_ = false;
    bool paused_  = false;
    // Which mechanism last paused mpv. A resume MUST use the same one: IPC
    // "cycle pause" sent to a SIGSTOP'd process just sits unread in its
    // socket buffer (the process is OS-suspended, not merely internally
    // paused), silently desyncing app state from mpv forever after.
    bool paused_via_signal_ = false;
