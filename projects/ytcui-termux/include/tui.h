...
public:
    ...
    bool init();
    void shutdown();
    void render(AppState& state, const Library* lib);
    void get_dimensions(int& w, int& h);
    // Dynamically enable/disable SGR mouse click reporting. No-op if the
    // terminal never supported it to begin with (mouse_enabled_ false).
    // Needed anywhere a screen wants raw taps back for something other than
    // mouse clicks -- most importantly Termux, where taps are swallowed as
    // click escape sequences the whole time mouse tracking is on, and
    // Android's on-screen keyboard never appears for a text field that
    // never sees a plain touch. Turn it off while the user needs to type
    // (streamlined Search) and back on for tap-to-navigate screens.
...
private:
    ...
    void render_streamlined(AppState& state, const Library* lib);
    ...
    void stream_playing(AppState& state);
    void stream_minimenu(const AppState& state);
    void stream_playlist_pick(const AppState& state);
    void stream_new_playlist_name(const AppState& state);
    void stream_timer_input(const AppState& state);
    // show_back: renders a tappable "‹" at the top-left (row 0) -- every
    // screen except the root Menu wants this, touch-only back navigation.
    // show_menu: tappable "Menu≡" at the top-right -- only screens with a
    // concrete video in view (Browse-with-selection, Playing) offer it.
    void stream_header(const AppState& state, const std::string& title,
                        bool show_back = true, bool show_menu = false);
